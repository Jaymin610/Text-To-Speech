...
def convert_to_speech_GTTS(is_preview, lang, desc, dir, mobile):
    ...
    if is_preview:
        ...
        dest = path + "\\user\\songdir\\" + dir + "\\" + str(mobile) + ".mp3"
        shutil.move(src, dest)
        return dir + "/" + str(mobile) + ".mp3"


def convert_to_speech_PTTS(is_preview, gender, desc, dir, speech_rate, mobile):
    ...
    if is_preview:
        ...
        engine.save_to_file(desc, path + "\\user\\songdir\\" + dir + "\\" + str(mobile) + ".mp3")
        engine.runAndWait()
        return dir + "/" + str(mobile) + ".mp3"
